...
def plot_ml_results(models, results, X_test_dict, y_test_dict, targets, target_names):
    """
    Create ML comparison plots
    """
    plots = {}
    ...
    # Figure 4: Prediction vs Real lines
    fig4, axes = plt.subplots(3, 1, figsize=(15, 15))
    
    n_show = min(200, len(X_test_dict[targets[0]]))
    idx_show = np.random.choice(len(X_test_dict[targets[0]]), n_show, replace=False)
    
    for i, target in enumerate(targets):
        ax = axes[i]
        
        y_true = y_test_dict[target][idx_show]
        sorted_idx = np.argsort(y_true)
        y_true_sorted = y_true[sorted_idx]
        
        for metodo, color, label in zip(['RF', 'PNN', 'CNN'], ['blue', 'green', 'red'],
                                        ['Random Forest', 'PNN', 'CNN']):
            if metodo == 'RF':
                y_pred = models['RF'][target].predict(X_test_dict[target][idx_show])[sorted_idx]
            elif metodo == 'PNN':
                y_pred = models['PNN'][target].predict(X_test_dict[target][idx_show])[sorted_idx]
            else:
                if TENSORFLOW_AVAILABLE:
                    X_test_cnn = X_test_dict[target][idx_show].reshape(-1, X_test_dict[target].shape[1], 1)
                    y_pred = models['CNN'][target].predict(X_test_cnn, verbose=0).flatten()[sorted_idx]
                else:
                    y_pred = models['CNN'][target].predict(X_test_dict[target][idx_show])[sorted_idx]
            
            ax.plot(y_true_sorted, y_pred, 'o-', markersize=3, linewidth=1,
                    color=color, alpha=0.7, label=label)
        
        min_val = y_true_sorted.min()
        max_val = y_true_sorted.max()
        ax.plot([min_val, max_val], [min_val, max_val], 'k--', linewidth=2, alpha=0.5, label='Ideal')
    ...
    plt.suptitle('Predictions vs Real: ML Methods Comparison', fontsize=16, fontweight='bold', y=1.02)
    plt.tight_layout()
    plots['prediction_lines'] = fig4
    
    # Figure 5: Boxplot of errors
    ...
    for i, target in enumerate(targets):
        ...
        ax.boxplot(errores, labels=labels, patch_artist=True,
    ...
    if uploaded_file is not None:
        try:
            ...
            if mode in ["Anisotropy & ML Prediction", "Combined Analysis"]:
                ...
                with col1:
                    st.metric("VP(0)", f"{df_aniso['VP_0'].mean():.0f} m/s")
                with col2:
                    st.metric("VP(45)", f"{df_aniso['VP_45'].mean():.0f} m/s")
                with col3:
                ...
                ax1.plot(x_vals, df_aniso['VP_0'], label='VP(0)', linewidth=2, color='blue')
                ax1.plot(x_vals, df_aniso['VP_45'], label='VP(45)', linewidth=2, color='green')
                ax1.plot(x_vals, df_aniso['VP_90'], label='VP(90)', linewidth=2, color='red')
                ax1.set_xlabel('Depth' if depth_col else 'Sample')
                ax1.set_ylabel('Velocity (m/s)')
                ax1.set_title('Anisotropic Velocities')
                ax1.legend()
                ...
                if 'anisotropy_epsilon' in df_aniso.columns:
                    ax2.plot(x_vals, df_aniso['anisotropy_epsilon'], label='ε (%)', linewidth=2, color='orange')
                if 'anisotropy_delta' in df_aniso.columns:
                    ax2.plot(x_vals, df_aniso['anisotropy_delta'], label='δ (%)', linewidth=2, color='purple')
                ax2.set_xlabel('Depth' if depth_col else 'Sample')
                ...
                if 'VP_variation' in df_aniso.columns:
                    ax3.plot(x_vals, df_aniso['VP_variation'], linewidth=2, color='darkred')
                    ax3.axhline(y=0, color='k', linestyle='--', alpha=0.3)
                    ax3.set_xlabel('Depth' if depth_col else 'Sample')
                    ax3.set_ylabel('VP Variation (%)')
                    ax3.set_title('VP Variation (VP90 - VP0) / VP0 * 100')
                    ax3.grid(True, alpha=0.3)
                
                # VP(45) vs VP(0) comparison
                ax4 = axes[1, 1]
                ax4.scatter(df_aniso['VP_0'], df_aniso['VP_45'], alpha=0.5, s=10, c='green')
                ...
                if 'run_ml' in locals() and run_ml:
                    with st.spinner("🚀 Training Machine Learning models..."):
                        ...
                        if len(targets) == 0:
                            ...
                            st.subheader("📈 ML Model Comparison Plots")
                ...
                **Integrated Analysis**: This mode combines rock physics modeling with anisotropic velocity prediction.
                ...
                if 'logs' in locals() and 'df_aniso' in locals():
                    ...
                    if 'IP_FRMMIX' in logs.columns and 'VP_0' in df_aniso.columns:
                        ...
                        ax.set_title('Rock Physics IP vs Anisotropic Velocities')
                        ax.legend()
                        ax.grid(True, alpha=0.3)
